 ...
 * Add this script to your GitHub Pages site to enable live location tracking
 * https://ashutosh231.github.io/rmsgenerator/
 */
...
function trackUserLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            localStorage.setItem(trackingId, JSON.stringify(locationData));
            ...
            fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${position.coords.latitude}&lon=${position.coords.longitude}`)
                ...
                .then(data => {
                    if (data && data.address) {
                        const locationInfo = JSON.parse(localStorage.getItem(trackingId));
                        locationInfo.address = {
                            city: data.address.city || data.address.town || data.address.village || 'Unknown',
                            state: data.address.state || data.address.county || 'Unknown',
                            full: data.display_name || 'Unknown'
                        };
                        localStorage.setItem(trackingId, JSON.stringify(locationInfo));
                    }
                })
                .catch(e => console.error('Address lookup failed', e));
                
            // Set up periodic location updates
            setTimeout(trackUserLocation, 60000); // Update every minute
        }, 
        function(error) {
